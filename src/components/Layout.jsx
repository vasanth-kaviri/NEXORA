...
export default function Layout() {
  ...
  return (
    <div className="app-container transform-gpu" style={{ transform: 'translateZ(0)' }}>
      <main className="page-content transform-gpu will-change-transform" style={{ padding: 0, paddingBottom: showNav ? 'calc(80px + var(--space-lg))' : (location.pathname === '/chatbot' ? 0 : 'var(--space-lg)'), backfaceVisibility: 'hidden' }}>
        {showBackButton && (
          <div style={{ padding: 'var(--space-md) var(--space-lg)', position: 'sticky', top: 0, zIndex: 50, backgroundColor: 'var(--bg-main)', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center' }}>
            <button 
              onClick={() => navigate(-1)} 
              className="flex items-center gap-xs text-primary interactive" 
              style={{ fontWeight: 600, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, margin: 0 }}
            >
              <ArrowLeft size={20} /> Back
            </button>
          </div>
        )}
        <div style={{ padding: location.pathname === '/chatbot' ? 0 : 'var(--space-lg)', paddingTop: showBackButton ? 'var(--space-md)' : (location.pathname === '/chatbot' ? 0 : 'var(--space-lg)') }}>
          <Outlet />
        </div>
      </main>
